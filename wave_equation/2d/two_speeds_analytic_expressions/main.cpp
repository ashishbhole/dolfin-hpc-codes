...
struct Speed : public Value< Speed, 1 >
{
  void eval( real * values, const real * x ) const
  {
    if(x[1] <= 0.0)
...
struct Bottom : public SubDomain
{
  bool inside(const real* x, bool on_boundary) const
  {
    return x[1] <= 0.0;
...
{ 
  ...
  Analytic<Speed> c( mesh, speed);

  real Nc = 0.05;
  real h = MeshQuality(mesh).h_max;
  tstep = Nc * h / speed0;
  Constant dt(tstep);
  Constant alpha_m(alpha_m_value);
  Constant alpha_f(alpha_f_value);
  WaveEquation::BilinearForm a(mesh, dt, alpha_m, alpha_f, c);
  ...
  WaveEquation::LinearForm L(mesh, u_old, v_old, a_old, ff, dt, alpha_m, alpha_f, c);
  Matrix A;
  Vector b;
  a.assemble(A, true);
